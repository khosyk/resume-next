...
import { demoRouteGroups, demoRoutes } from "../config/routes";

// Lab 공통 상단 네비
export function DemoLabHeader() {
...
interface DemoHubSectionsProps {
	currentHref?: string;
}

// Hub 본문용 섹션별 카드 네비
export function DemoHubSections({ currentHref = "/demos" }: DemoHubSectionsProps) {
	return (
		<div className="space-y-10">
			{demoRouteGroups
				.filter((group) => group.id !== "hub")
				.map((group) => {
					const routes = demoRoutes.filter((r) => r.group === group.id);
