...
> 마스터 계획: [DEMO_HUB_PLAN.md](./DEMO_HUB_PLAN.md)  
...
## Week 1 — Next 이전 + 메인 살리기 (진행 중)

**목표:** Vite 대비 기능 동일한 `/` + Lab 진입 골격
...
- [x] `App.tsx` → `src/app/page.tsx` (`"use client"` on App)
- [x] `domain/`, `shared/` import 경로 유지
- [x] `layout.tsx` metadata (title, description)
...
- [x] `/demos` placeholder 페이지

### 1.5 Week 1 완료 기준

- [x] `/` Next.js로 빌드·동작
- [x] Lab 링크 3곳 + `/demos` placeholder
...
## Week 2 — Demo Hub + 렌더링 4종 (진행 중)

**목표:** SSR/SSG/ISR/CSR 동작 + Why/How/Verify 카피 — **네비·골격 완료, 콘텐츠는 함께 작성**

### 2.1 공통

- [x] `src/features/demos/config/routes.ts` — 라우트·prev/next 정의
- [x] `DemoLabHeader` — 상단 탭 네비 (Hub · SSR · … · 3D)
- [x] `DemoPageShell` — 전략 뱃지 · prev/next · 콘텐츠 placeholder
- [x] `src/app/demos/layout.tsx`

### 2.2 Hub

- [x] `/demos` — Rendering / Visualization 섹션 카드 네비

### 2.3 페이지 골격 (콘텐츠 TBD)

- [x] `/demos/ssr` · `/demos/ssg` · `/demos/isr` · `/demos/csr`
- [x] `/demos/charts` · `/demos/3d`
- [ ] `portfolioMetrics.ts`
...
- [ ] `/demos/charts` — `dynamic(ssr: false)` 적용
...
- [ ] Hero·Stats Server Component 분리
- [ ] OG metadata 보강
